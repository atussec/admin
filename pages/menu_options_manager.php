...
################################################################################
    ...
    $mode = isset($_GET['meno_mode']) ? removeBadChars($_GET['meno_mode']) : "";
    $rid = isset($_GET['meno_rid']) ? (int)$_GET['meno_rid'] : "";
    $msg = isset($_GET['msg']) ? removeBadChars($_GET['msg']) : "";
    $mgid = isset($_GET['mgid']) ? (int)$_GET['mgid'] : "";
    $moid = isset($_GET['moid']) ? (int)$_GET['moid'] : "";
    ...
    if($mode == "delete"){
        ...
        if($r__ = $db->fetchAssoc()){
            if(intval($r__['is_removable']) == intval("0")){
                echo "<script>window.location.href='menu_options_manager.php?msg=1&moid=".$moid."&mgid=".$mgid."';</script>";
                exit;
            }
        }
    }
...
if($msg == "1"){
    echo "<center><font color='#009a00'>This menu option cannot be removed, only hidden!</font></center><br>";
}
